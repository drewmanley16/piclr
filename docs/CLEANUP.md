...
- [x] **Adopt the existing `SectionHeader`** — 1 adopter → 12. Added `= nil` defaults to `actionTitle`/`action`, plus an optional `systemImage` so the one `Label`-based header could adopt too. Two of the ~13 sites in the original list are **not** section headers and were deliberately left alone: `GearSheets:197` is a row title with a subtitle beneath it, and `ProfileView:390` carries a trailing "Details" + chevron *inside* an enclosing `Button`, so adopting would nest buttons and change the tap target. **S**
