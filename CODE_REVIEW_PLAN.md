...
- [ ] **1.11 Adopt `os.Logger`.** Exactly 5 bare `print()` calls and no `Logger` anywhere (`PushService.swift:89`, `LiveActivityManager.swift:48`, `AppStore.swift:377,2287,2296`). Mechanical swap to per-subsystem loggers (`Push`, `LiveActivity`, `FeedPerf`) — the only way to debug TestFlight builds via Console.app. Effort: **S**. Risk: low.
- [ ] **1.12 Wrap edge-function handlers in a top-level try/catch.** All four functions in `supabase/functions/` only guard `req.json()`; an unexpected throw yields Deno's default 500 instead of the functions' own `json({error},500)` shape (verified in `delete-account/index.ts:11–56`). Effort: **M** (4 small files). Risk: low.
